...
class polarisation_mosaic:
    """
    Class to produce polarisation mosaics in Stokes Q and U.
    """
    ...
    def go(self):
        ...
        self.correct_polcubes()
        veri = self.check_polimages()
        if (~veri).all():
            raise UnboundLocalError('All beams are rejected. Not continuing with polarisation mosaicking!')
#            print('All beams are rejected. Not continuing with mosaicking!')
        else:
            utils.copy_polimages(self, veri)
            utils.copy_polbeams(self)
            if self.pol_mode_fixed_fwhm == '':
                cbeam = utils.get_common_psf(self, veri, format='array')
            elif self.pol_mode_fixed_fwhm >= 0.0:
                cbeam = utils.get_common_psf(self, veri, format='pol-fixed')
    ...
    def correct_polcubes(self):
        """
        Correct the Stokes Q- and U-cubes to represent the correct fluxes depending on the used polarisation calibrator
        """
        for b in range(40):
            if os.path.isfile(os.path.join(self.basedir, self.obsid) + '/param_' + str(b).zfill(2) + '_preflag_3C138.npy'):
                if os.path.isfile(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/polcorr.txt')):
                    print('Cubes for Beam ' + str(b).zfill(2) + ' have already been corrected!')
                else:
                    # Exchange Q and U values
                    os.rename(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube.fits'), os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube_copy.fits'))
                    os.rename(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Ucube.fits'), os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube.fits'))
                    os.rename(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube_copy.fits'), os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Ucube.fits'))
                    pathlib.Path(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/polcorr.txt')).touch()
            elif os.path.isfile(os.path.join(self.basedir, self.obsid) + '/param_' + str(b).zfill(2) + '_preflag_3C286.npy'):
                if os.path.isfile(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/polcorr.txt')):
                    print('Cubes for Beam ' + str(b).zfill(2) + ' have already been corrected!')
                else:
                    # Exchange Q and U values
                    ...
                    os.rename(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Ucube.fits'), os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube.fits'))
                    os.rename(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube_copy.fits'), os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Ucube.fits'))
                    # In case of 3C286, change the sign of the new Q-values
                    qcube = pyfits.open(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube.fits'))
                    qhdu = qcube[0]
                    qhdr = qhdu.header
                    qdata = qhdu.data
                    pyfits.writeto(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/Qcube.fits'), data=-1.0*qdata, header=qhdr, overwrite=True)
                    pathlib.Path(os.path.join(self.basedir, self.obsid, str(b).zfill(2), 'polarisation/polcorr.txt')).touch()
            else:
                print('Cubes for Beam ' + str(b).zfill(2) + ' are not available. Cannot correct Stokes values to correct polarisation angles!')
    ...
    def make_polmosaic(self, qimages, uimages, pbimages, sb, psf, reference=None, pbclip=None):
        ...
        for qimg, uimg, pb in zip(qimages, uimages, pbimages):
            ...
            qtmpimg = fm.fits_squeeze(qtmpimg)  # remove extra dimensions
            utmpimg = fm.fits_squeeze(utmpimg)  # remove extra dimensions
            ...
            qtmppb = fm.fits_squeeze(qtmppb)  # remove extra dimensions
            utmppb = fm.fits_squeeze(utmppb)  # remove extra dimensions
