...
class continuum_mosaic:
    ...
    def go(self):
        ...
        if len(images) == 0:
            raise ZeroBeamError('All beams are rejected. Not continuing with continuum mosaicking!')
        if len(images) == 1:
            raise OneBeamError('Only one beam was accepted. Not continuing with continuum mosaicking!')
        else:
    ...
    def make_contmosaic(self, images, pbimages, reference=None, rmnoise=False):
        ...
        """
        # Get the common psf
        if self.cont_mode_fixed_fwhm == '':
            common_psf = utils.get_common_psf(self, images, format='fits')
        elif self.cont_mode_fixed_fwhm >= 0.0:
